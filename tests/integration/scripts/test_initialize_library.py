...
def test_init_library(mongo_host):
    ...
    with patch('giantarctic.scripts.arctic_init_library.do_db_auth', return_value=True), \
...
def test_init_library_bad_library(mongo_host):
    with pytest.raises(Exception):
        with patch('giantarctic.arctic.get_auth', return_value=Credential('admin', 'adminuser', 'adminpwd', 'admin')), \
             patch('pymongo.database.Database.authenticate', return_value=True), \
